...
describe DockingStation do
  ...
  describe '#release_bike' do
    ...
    it 'expect an error if only broken bikes available' do
      bike = Bike.new
      bike.report_broken
      subject.dock(bike)
      expect { subject.release_bike }.to raise_error('No bikes available')
    ...
    it 'should not release a broken bike if working bike available' do
      bike1 = Bike.new
      subject.dock(bike1)
      bike2 = Bike.new
      bike2.report_broken
      subject.dock(bike2)
      expect(subject.release_bike).not_to eq(bike2)
    end
  end
